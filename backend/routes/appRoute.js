...
function secondtimecart(req, res, oldcart, newfood, userid) {
    var oldavail = false;
   
    var newtotal = oldcart.total + newfood.foodprice;
    var tot;
   
    var oldfoodjsonarr = oldcart['food']
   
    for (var i = 0; i < oldfoodjsonarr.length; i++) {
        if (oldfoodjsonarr[i]._id == newfood._id) {
            oldavail = true;
        }
    }
    ...
    if (oldavail) {
        ...
        for (var i = 0; i < oldfoodjsonarr.length; i++) {
            if (oldfoodjsonarr[i]._id == newfood._id) {
                oldfoodjsonarr[i].qty += 1;
                oldcart.total += oldfoodjsonarr[i].foodprice
                tot = oldcart.total
                console.log(oldcart.total);
            }
        }

        console.log(oldfoodjsonarr);
...
router.get('/getcartitem', verifyToken, (req, res, next) => {
    Cart.find({ whichuser: req.userId }, (err, foods) => {
        if (err) {
...
function deletefromcart(req, res, oldcart, newfood, userid) {
    var temp = [];
    var total;
    var oldfoodsonarr = oldcart['food']
    ...
    if (total == 0) {
        ...
        res.json({ msg: "food deleted from the cart" })
